...
for i in range(num_nuevas):
    while True:
        nuevapalabra = input(f"Inserte la palabra numero {str(i+1)}: {str(palabras)} \n=> ")
...
while True: 
    num_eliminar = input(f"Cuantas palabras desea eliminar? {str(palabras)}\n=> ")
...
for i in range(num_eliminar):
    while True:
        palabra = input(f"{str(i+1)}.Inserte la palabra a eliminar:{str(palabras)} \n=> ")
